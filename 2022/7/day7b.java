...
public class day7b {
  ...
  public static void main(String[] args) {
    ...
    int spaceUsed = dir.calcFileSizes(dir.fileTreeRoot);
   
    int spaceLeft = 70000000 - spaceUsed;
    int spaceNeeded = 30000000 - spaceLeft;
    System.out.println(spaceNeeded);

    System.out.println(dir.findSmallestAbove(dir.fileTreeRoot,spaceNeeded));
  ...
  private int findSmallestAbove(Node<File> start, int limit)
  {
    Collection<Node<File>> list = start.getChildren();
    int smallest = start.getData().getSize();
    
    for (Node<File> child : list) {
      
      int size = findSmallestAbove(child, limit);
      if(size < smallest && size > limit)
        smallest = size; 
